...
const UserProfilePage: React.FC = () => {
  ...
  const { data: profilePhotoBlob, refetch: refetchPhoto } = useProfilePhoto();
  const uploadPhotoMutation = useUploadProfilePhoto();
  const deletePhotoMutation = useDeleteProfilePhoto();
  const updateProfileMutation = useUpdateProfile();

  // Create object URL for the profile photo blob
  ...
  // Load photo when user data is available and has profile_photo_url
  useEffect(() => {
    if (user?.profile_photo_url && !profilePhotoBlob) {
      refetchPhoto();
    }
  ...
  const handleUploadPhoto = async () => {
    if (selectedFile) {
      try {
        ...
        // Refetch both user data and photo blob
        refetchUser();
        setTimeout(() => refetchPhoto(), 100); // Small delay to ensure user data is updated first
      } catch (error) {
  ...
  const handleDeletePhoto = async () => {
    try {
      ...
      refetchUser();
      setPhotoObjectUrl(null); // Clear the photo immediately
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-emerald-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-red-700 font-medium">Error al cargar el perfil</p>
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    ...
                    <div className="flex gap-2">
                      <motion.button
                        ...
                        className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium disabled:opacity-50"
                      >
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    ...
                    {photoObjectUrl && (
                      <button
                        ...
                        className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-medium disabled:opacity-50"
                      >
            ...
            >
              ...
              <div className="flex gap-3 mt-6">
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white py-2 px-4 rounded-lg font-medium disabled:opacity-50 flex items-center justify-center gap-2"
                >
