...
export function usePlantMetricTrend(
  ...
  // Transformar datos para gráficos de tendencias
...
// Hook para múltiples tendencias (útil para dashboard)
export function usePlantTrends(plantId: string, hours: number = 24) {
  const temperature = usePlantMetricTrend(plantId, 'temperature', hours);
  const humidity = usePlantMetricTrend(plantId, 'air_humidity', hours);
  const light = usePlantMetricTrend(plantId, 'light_intensity', hours);

  return {
    temperature,
    humidity,
    light,
    isLoading: temperature.isLoading || humidity.isLoading || light.isLoading,
    hasError: !!(temperature.error || humidity.error || light.error),
  };
}
